...
Each board can be shared with others via a unique shareable link. Links can be created with one of two permission levels:
...
3. In the popup, choose **Edit access** or **Read-only access** and click **Create link**.
4. The link is automatically copied to your clipboard — send it to anyone you'd like to share with.

Recipients can open the link in their browser without needing an account. Edit-link recipients can make changes that are saved server-side and immediately reflected for the board owner as well.

> **Note:** Share links are persistent — they remain valid until manually removed. Each board supports one link per permission level.
